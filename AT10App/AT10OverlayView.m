...
- (void)dotTapped { [self toggleCollapse]; }
...
- (void)buildPanel {
    ...
    [_collapseBtn addTarget:self action:@selector(toggleCollapse) forControlEvents:UIControlEventTouchUpInside];
    [hdr addSubview:_collapseBtn];

    UIPanGestureRecognizer *panG = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePanelPan:)];
    ...
    [self setButtonBlue];
    [_toggleBtn addTarget:self action:@selector(toggleTapped) forControlEvents:UIControlEventTouchUpInside];
    ...
    [addDot addTarget:self action:@selector(addDotTapped) forControlEvents:UIControlEventTouchUpInside];
...
- (void)addDotTapped {
    ...
    CGPoint c = CGPointMake(self.bounds.size.width/2 + (_dots.count * 15), self.bounds.size.height/2);
