...
static inline CGPoint ZeroPoint(void) { return CGPointMake(0.0, 0.0); }
static inline CGRect  ZeroRect(void)  { return CGRectMake(0.0, 0.0, 0.0, 0.0); }
...
+ (instancetype)sharedOverlay {
    ...
    dispatch_once(&t, ^{ i = [[self alloc] initWithFrame:ZeroRect()]; });
    return i;
}
...
- (void)handleDotPan:(UIPanGestureRecognizer *)g {
    ...
    _dotPos = _dot.center;
    [g setTranslation:ZeroPoint() inView:self];
}
...
- (void)buildPanel {
    ...
    UILabel *spdTitle = [self lbl:@"السرعة" x:10 y:y w:100 bold:YES];
    spdTitle.textColor = [UIColor colorWithRed:0.48 green:0.70 blue:0.88 alpha:1];
    [_panelBody addSubview:spdTitle];

    _speedValLabel = [self lbl:@"اقصى سرعة" x:110 y:y w:90 bold:YES];
...
- (UILabel *)lbl:(NSString *)t x:(int)x y:(int)y w:(int)w bold:(BOOL)b {
    UILabel *l = [[UILabel alloc] initWithFrame:CGRectMake(x,y,w,18)];
    l.text = t;
    l.textColor = BLUE_DARK;
    l.font = b ? [UIFont boldSystemFontOfSize:9.5] : [UIFont systemFontOfSize:9];
    return l;
}
...
- (void)speedChanged {
    ...
    _cps = v;
    _speedValLabel.text = v >= 120 ? @"اقصى سرعة" : [NSString stringWithFormat:@"%d ن/ث", v];
}
...
- (void)toggleTapped {
    if (!_running) {
        ...
        [self setButtonBlack];
        [_toggleBtn setTitle:@"ايقاف" forState:UIControlStateNormal];
    } else {
        _running = NO;
